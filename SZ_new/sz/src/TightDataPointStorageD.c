...
int new_TightDataPointStorageD_fromFlatBytes(TightDataPointStorageD **this, unsigned char* flatBytes, int flatBytesLength)
{
    ...
    for (i = 0; i < 4; i++)
        byteBuf[i] = flatBytes[index++];
    (*this)->bitArrayLength = bytesToInt_bigEndian(byteBuf); //4
    ...
    (*this)->bitArray = (unsigned char*)malloc(sizeof(unsigned char)*(*this)->bitArrayLength);
	...
	if ((*this)->rtypeArray != NULL) 
	{
		(*this)->residualMidBits_size = flatBytesLength - 3 - 4 - 1 - 4 - radExpoL - segmentL - pwrErrBoundBytesL - 4 - 8 - 1 - 8 - 4 - 4 - 4 - 4
				...
				- (*this)->exactMidBytes_size - pwrErrBoundBytes_size - (*this)->bitArrayLength - 4;
	...
	else
	{
		(*this)->residualMidBits_size = flatBytesLength - 3 - 4 - 1 - 4 - radExpoL - segmentL - pwrErrBoundBytesL - 4 - 8 - 1 - 8 - 4 - 4
				...
				- (*this)->leadNumArray_size - (*this)->exactMidBytes_size - pwrErrBoundBytes_size - (*this)->bitArrayLength - 4;
	}	
    ...
    memcpy((*this)->bitArray, &flatBytes[index], (*this)->bitArrayLength*sizeof(char));
    index+=(*this)->bitArrayLength* sizeof(char);
...
void new_TightDataPointStorageD(TightDataPointStorageD **this, 
	...
	*this = (TightDataPointStorageD *)malloc(sizeof(TightDataPointStorageD));
	...
	printf("before huffman:%d, after huffman:%d\n", dataSeriesLength, (*this)->typeArray_size);
...
void new_TightDataPointStorageD_alter(TightDataPointStorageD **this,
								      int dataSeriesLength, int exactDataNum, unsigned char* bitArray, int bitArrayLength,
								      int* type, int quantLength, unsigned char* exactMidBytes, int exactMidBytes_size,
								      unsigned char* leadNumIntArray,  //leadNumIntArray contains readable numbers....
								      unsigned char* resiMidBits, int resiMidBits_size,
								      unsigned char* resiBitLength, int resiBitLengthSize,
								      double realPrecision, double medianValue, char reqLength, unsigned int intervals,
	...
	*this = (TightDataPointStorageD *)malloc(sizeof(TightDataPointStorageD));
    ...
    (*this)->typeArray_size = quantLength* sizeof(int16_t);

	/* 如果量化区间的参数设置的比较大，则直接按照int型保存
	(*this)->typeArray = (unsigned char*)malloc(quantLength*sizeof(int));
...
void convertTDPStoBytes_double_alter(TightDataPointStorageD* tdps, unsigned char* bytes, unsigned char* dsLengthBytes, unsigned char sameByte)
{
    int i, k = 0;
    unsigned char intervalsBytes[4];
    unsigned char typeArrayLengthBytes[4];
    ...
    unsigned char bitArrayLengthBytes[4];
    ...
    intToBytes_bigEndian(bitArrayLengthBytes, tdps->bitArrayLength);
    for (i = 0; i < 4; i++)//4
        bytes[k++] = bitArrayLengthBytes[i];
    ...
    memcpy(&(bytes[k]), tdps->bitArray, tdps->bitArrayLength);
    k += tdps->bitArrayLength;
...
void convertTDPStoBytes_double_reserve_alter(TightDataPointStorageD* tdps, unsigned char* bytes, unsigned char* dsLengthBytes, unsigned char sameByte) {
	int i, k = 0;
	unsigned char intervalsBytes[4];
	unsigned char typeArrayLengthBytes[4];
	unsigned char rTypeLengthBytes[4];
	...
	for (i = 0; i < 4; i++)//4
		bytes[k++] = bitArrayLengthBytes[i];
	memcpy(&(bytes[k]), tdps->bitArray, tdps->bitArrayLength * sizeof(int));
	k += tdps->bitArrayLength;
}
...
void convertTDPStoFlatBytes_double_alter(TightDataPointStorageD *tdps, unsigned char** bytes, int *size)
{
	int i, k = 0;
	unsigned char dsLengthBytes[4];
	...
	else if (tdps->rtypeArray == NULL)
	{
		...
		int totalByteLength = 3 + 4 + 1 + 4 + radExpoL + segmentL + pwrBoundArrayL + 4 + 8 + 1 + 8 + 4 + 4 + 4
							  + tdps->typeArray_size + tdps->leadNumArray_size
							  + tdps->exactMidBytes_size + residualMidBitsLength + tdps->pwrErrBoundBytes_size + tdps->bitArrayLength + 4;

		*bytes = (unsigned char *)malloc(sizeof(unsigned char)*totalByteLength);

		convertTDPStoBytes_double_alter(tdps, *bytes, dsLengthBytes, sameByte);

		*size = totalByteLength;

        printf("bitArrayLength:%d, typeArrayLength:%d, floatArrayLength:%d, errBoundLength:%d\n", tdps->bitArrayLength, tdps->typeArray_size, tdps->exactMidBytes_size, tdps->pwrErrBoundBytes_size);
	...
	else //the case with reserved value
	{
		...
		*bytes = (unsigned char*)malloc(sizeof(unsigned char)*totalByteLength);
		...
		convertTDPStoBytes_double_reserve_alter(tdps, *bytes, dsLengthBytes, sameByte);
