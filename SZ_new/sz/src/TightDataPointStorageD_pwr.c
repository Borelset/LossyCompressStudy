...
#include <stdio.h>
#include <string.h>
#include <sz/include/FitMethods.h>
...
void decompressDataSeries_double_1D_pwr(double** data, int dataSeriesLength, TightDataPointStorageD* tdps) 
{
	...
	*data = (double*)malloc(sizeof(double)*dataSeriesLength);
	...
	int16_t* type = (int16_t*)tdps->typeArray;
	int typeIndex = 0;
	struct BitHolder* bitHolder = (struct BitHolder*)tdps->bitArray;
	...
	FILE* compressionLog = fopen("decompressionLog", "w");
	...
	for (i = 0; i < dataSeriesLength; i++) 
	{
		...
		if(i % 8 == 0){
		    struct BitHolder* ptr = BitHolderGetByte(bitHolder, i);
			memset(writeBuffer, 0, 1024);
			sprintf(writeBuffer, "BitHolder[%d]:%x %x %x\n", i/8, ptr->holder[0], ptr->holder[1], ptr->holder[2]);
			fwrite(writeBuffer, 1, 1024, compressionLog);
		}
		switch (BitHolderGetRecord(bitHolder, i)) {
            case 0:
                predValue = methodArray[0]((*data)[i-3], (*data)[i-2], (*data)[i-1]);
                (*data)[i] = predValue;
				memset(writeBuffer, 0, 1024);
				sprintf(writeBuffer, "%d: compressed%e\ttype:%d\n", i, predValue, 0);
				fwrite(writeBuffer, 1, 1024, compressionLog);
                break;
            case 1:
                predValue = methodArray[1]((*data)[i-3], (*data)[i-2], (*data)[i-1]);
                (*data)[i] = predValue;
				memset(writeBuffer, 0, 1024);
				sprintf(writeBuffer, "%d: compressed%e\ttype:%d\n", i, predValue, 1);
				fwrite(writeBuffer, 1, 1024, compressionLog);
                break;
            case 2:
                predValue = methodArray[2]((*data)[i-3], (*data)[i-2], (*data)[i-1]);
                (*data)[i] = predValue;
				memset(writeBuffer, 0, 1024);
				sprintf(writeBuffer, "%d: compressed%e\ttype:%d\n", i, predValue, 2);
				fwrite(writeBuffer, 1, 1024, compressionLog);
                break;
            case 3:
                predValue = methodArray[3]((*data)[i-3], (*data)[i-2], (*data)[i-1]);
                (*data)[i] = predValue;
				memset(writeBuffer, 0, 1024);
				sprintf(writeBuffer, "%d: compressed%e\ttype:%d\n", i, predValue, 3);
				fwrite(writeBuffer, 1, 1024, compressionLog);
                break;
            case 4:
                predValue = methodArray[4]((*data)[i-3], (*data)[i-2], (*data)[i-1]);
                (*data)[i] = predValue;
				memset(writeBuffer, 0, 1024);
				sprintf(writeBuffer, "%d: compressed%e\ttype:%d\n", i, predValue, 4);
				fwrite(writeBuffer, 1, 1024, compressionLog);
                break;
            case 5:
                predValue = methodArray[5]((*data)[i-3], (*data)[i-2], (*data)[i-1]);
                (*data)[i] = predValue;
				memset(writeBuffer, 0, 1024);
				sprintf(writeBuffer, "%d: compressed%e\ttype:%d\n", i, predValue, 5);
				fwrite(writeBuffer, 1, 1024, compressionLog);
            ...
            case 7:
				...
				memset(writeBuffer, 0, 1024);
				sprintf(writeBuffer, "%d: compressed%e\ttype:%d\n", i, exactData + medianValue, 7);
				fwrite(writeBuffer, 1, 1024, compressionLog);
                break;
            case 6:
                predValue = 2 * (*data)[i-1] - (*data)[i-2];
                //predValue = (*data)[i-1];
                (*data)[i] = predValue + (type[typeIndex]-intvRadius)*interval;
				memset(writeBuffer, 0, 1024);
				sprintf(writeBuffer, "%d: compressed%e\ttype:%d\ttype:%d\tindex:%d\n", i, (*data)[i], 6, type[typeIndex], typeIndex);
				fwrite(writeBuffer, 1, 1024, compressionLog);
				typeIndex++;
                break;
		}
