...
int dataLength, int *outSize, double min, double max)
{
	...
	struct BitHolder bitHolder[BitHolderGetRequiredCount(dataLength)];
	memset(bitHolder, 0, sizeof(bitHolder));
	
	FILE* compressionLog = fopen("compressionLog", "w");
    ...
    BitHolderAddRecord(bitHolder, 0, 7);
    memset(writeBuffer, 0, 1024);
    sprintf(writeBuffer, "%d: ori:%e\tcompressed%e\ttype:%d\trate:%f\n", 0, spaceFillingValue[0], vce->data, 7, spaceFillingValue[0]/vce->data);
    fwrite(writeBuffer, 1, 1024, compressionLog);
	...
	addExactData(exactMidByteArray, exactLeadNumArray, resiBitArray, lce);
	listAdd_double(last3CmprsData, vce->data);
    BitHolderAddRecord(bitHolder, 1, 7);
    memset(writeBuffer, 0, 1024);
    sprintf(writeBuffer, "%d: ori:%e\tcompressed%e\ttype:%d\trate:%f\n", 1, spaceFillingValue[1], vce->data, 7, spaceFillingValue[1]/vce->data);
    fwrite(writeBuffer, 1, 1024, compressionLog);
	//printf("%.30G\n",last3CmprsData[0]);

	//add the third data
	addDBA_Data(resiBitLengthArray, (unsigned char)resiBitsLength);
	compressSingleDoubleValue(vce, spaceFillingValue[2], realPrecision, medianValue, reqLength, reqBytesLength, resiBitsLength);
	updateLossyCompElement_Double(vce->curBytes, preDataBytes, reqBytesLength, resiBitsLength, lce);
	memcpy(preDataBytes,vce->curBytes,4);
	addExactData(exactMidByteArray, exactLeadNumArray, resiBitArray, lce);
	listAdd_double(last3CmprsData, vce->data);
    BitHolderAddRecord(bitHolder, 2, 7);
    memset(writeBuffer, 0, 1024);
    sprintf(writeBuffer, "%d: ori:%e\tcompressed%e\ttype:%d\trate:%f\n", 2, spaceFillingValue[2], vce->data, 7, spaceFillingValue[2]/vce->data);
    fwrite(writeBuffer, 1, 1024, compressionLog);
	...
	for(i=3;i<dataLength;i++)
	{
		...
		double methodsMinErr = realPrecision;
		int methodsMinIndex = -1;
		double err = 0.0;
		for(int jj=0; jj < sizeof(methodArray) / sizeof(void*); jj++){
		    int test = sizeof(methodArray);
			err = methodArray[jj](last3CmprsData[2], last3CmprsData[1], last3CmprsData[0]) - curData;
			if(fabs(err) <= fabs(methodsMinErr)){
				methodsMinErr = err;
				methodsMinIndex = jj;
			}
		}

		if(methodsMinIndex != -1){
            BitHolderAddRecord(bitHolder, i, methodsMinIndex);
            pred = curData + methodsMinErr;
            listAdd_double(last3CmprsData, pred);
            hit++;
            memset(writeBuffer, 0, 1024);
            sprintf(writeBuffer, "%d: ori:%e\tcompressed%e\ttype:%d\trate:%f\n", i, curData, pred, methodsMinIndex, curData/pred);
            fwrite(writeBuffer, 1, 1024, compressionLog);
            continue;
		}
		if(predAbsErr<checkRadius)
		{
			...
			if(curData>=pred)
			{
				type[typeIndex] = intvRadius+state;
            ...
            memset(writeBuffer, 0, 1024);
            sprintf(writeBuffer, "%d: ori:%e\tcompressed%e\ttype:%d\trate:%f\ttype:%d\tindex:%d\n", i, curData, pred, 6, curData/pred, type[typeIndex], typeIndex);
            fwrite(writeBuffer, 1, 1024, compressionLog);
			listAdd_double(last3CmprsData, pred);
			BitHolderAddRecord(bitHolder, i, 6);
			quant++;
            typeIndex++;
			continue;
		}
		...
		//type[i] = 0;
		BitHolderAddRecord(bitHolder, i, 7);
		addDBA_Data(resiBitLengthArray, (unsigned char)resiBitsLength);
		...
		listAdd_double(last3CmprsData, vce->data);
        memset(writeBuffer, 0, 1024);
        sprintf(writeBuffer, "%d: ori:%e\tcompressed%e\ttype:%d\trate:%f\n", i, curData, vce->data, 7, curData/vce->data);
        fwrite(writeBuffer, 1, 1024, compressionLog);
	}//end of for

	printf("hit:%d, quant:%d, miss:%d\n", hit, quant, miss);
...
//	int expSegmentsInBytes_size = convertESCToBytes(esc, &expSegmentsInBytes);
	...
	new_TightDataPointStorageD_alter(&tdps, dataLength, exactDataNum, (unsigned char*)&bitHolder, sizeof(bitHolder),
			type, quant, exactMidByteArray->array, exactMidByteArray->size,
...
//	printf("tdps->typeArray_size=%d\n", tdps->typeArray_size);
	...
	convertTDPStoFlatBytes_double_alter(tdps, newByteData, outSize);
